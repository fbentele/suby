...
public class MainView implements PropertyChangeListener {
	...
	public void getMainView() {
		...
		interpretList.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				if (selection.length == 1) {
					...
					Integer id = Integer.parseInt(interpret.subSequence(interpret.indexOf("(") + 1,
							interpret.indexOf(")")).toString());
					albumTable.removeAll();
					songTable.removeAll();
					for (Album album : alb.getAllForArtist(id)) {
						TableItem titem = new TableItem(albumTable, SWT.NONE);
						titem.setText(new String[] { album.getId(), album.getName(),
								album.getYear() });
						Image albumArt;
						try {
							albumArt = new Image(display, album.getFullCoverArtPath());
						} catch (SWTException swte) {
							albumArt = new Image(display, App.class.getClassLoader()
		...
		play.setImage(new Image(display, App.class.getClassLoader().getResourceAsStream(
				"img/play.png")));
		...
		pause.setImage(new Image(display, App.class.getClassLoader().getResourceAsStream(
				"img/pause.png")));
		...
		previous.setImage(new Image(display, App.class.getClassLoader().getResourceAsStream(
				"img/previous.png")));
		...
		next.setImage(new Image(display, App.class.getClassLoader().getResourceAsStream(
				"img/next.png")));
	...
	private void updateAlbumTable() {
		...
		if (f.exists()) {
			...
			albumArt = new Image(display, App.class.getClassLoader().getResourceAsStream(
					"img/nocover.png"));
		}
	...
	private void updateCurrentlyPlaying() {
		...
		if (cur != null) {
			...
			try {
				...
				Image currentAlbumArt = new Image(display, App.class.getClassLoader()
						.getResourceAsStream("img/nocover.png"));
				currentAlbumArtContainer.setImage(currentAlbumArt);
			}
		}
