...
public class Albums {
	...
	public List<Album> getAllForArtist(Integer id) {
		...
		try {
			albumJson = obj.getJSONObject("subsonic-response").getJSONObject("artist")
					.getJSONArray("album");
		} catch (JSONException e) {
			if (e.getMessage().contains("not a JSONArray")) {
				albumJson = new JSONArray(Collections.singletonList(obj
						.getJSONObject("subsonic-response").getJSONObject("artist")
						.getJSONObject("album")));
			} else {
		...
		for (int i = 0; i < albumJson.length(); i++) {
			...
			try {
				...
				System.out.println("album err on interpret: " + id + " and album: "
						+ album.get("name"));
		...
		try {
			String error = obj.getJSONObject("subsonic-response").getJSONObject("error")
					.getString("message");
	...
	public Album getAlbum(String id) {
		JSONObject raw = rest.getJson("getAlbum.view", "id", id.toString())
				.getJSONObject("subsonic-response").getJSONObject("album");
